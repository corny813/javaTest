...
public class DataProcessing extends ApplicationFrame {
	...
	private JFreeChart createChart(final XYDataset dataset) {
		...
		final JFreeChart chart = ChartFactory.createXYLineChart(
				...
				"Time (sec) [Difference from original timestamp]", // x axis
																	// label
	...
	private XYDataset createDataset() {
		...
		for (String[] tempval : acc_values) {
			...
			if (Double.parseDouble(tempval[0]) == 0.0
					&& Double.parseDouble(tempval[1]) == 0.0
					&& Double.parseDouble(tempval[2]) == 0.0)
	...
	private void displayChart() {
		...
		if (this.frame == null) {
			...
			final JFreeChart chart = ChartFactory.createXYStepChart(title,
					xAxisLabel, yAxisLabel, data, PlotOrientation.VERTICAL,
					true, // legend
					true, // tooltips
					false // urls
					);
	...
	public static XYDataset createStepXYDataset() {
		...
		for (int i = 0; i < gps_values.size(); i++) {
			...
			if (j < gps_values.size()) {
				...
				/*
				 * Using Haversine formula for calculating distance between
				 * successive GPS co-ordinates and if difference is greater than
				 * a threashold, movement has occured
				 */
	...
	public static void main(String[] args) {
		...
		String logFilePath = "C:\\Users\\corny\\Desktop\\Sensors\\Android_Application-master\\Accel\\logdata.txt";
		Path path = Paths
				.get(logFilePath);
